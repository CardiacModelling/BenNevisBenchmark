...
def make_combined_plot(
    ...
    image2, fig2, ax2, meters2indices2 = make_image(
        ...
        alpha=alpha,
        boundaries=[ben_x - b * 2.125, ben_x + b * 2.125, ben_y - b * 1.65, ben_y + b * 1.65],
        zoom=1,
        labels=labels,
        image_only=False,
    )
...
def make_sequence(starting_points=[], ending_points=[], start_index=0, batch_size=None, downsampling=1):
    ...
    for i in tqdm.tqdm(range(start_index, N, downsampling)):
        canvas, fig1, ax1, meters2indices1, \
            fig2, ax2, meters2indices2, \
                fig3, ax3, meters2indices3 = make_combined_plot(
            starting_points=starting_points[i:i+downsampling] if starting_points else [], 
            ending_points=ending_points[i:i+downsampling] if ending_points else [],
            fig1=fig1,
            ax1=ax1,
...
def create_video_from_images(image_directory, output_video_path, codec='XVID', fps=30.0):
    ...
    first_image_path = os.path.join(image_directory, os.listdir(image_directory)[0])
...
def make_imgs(start_points=[], end_points=[], downsampling=1):
    ...
    # print(input_values)
    results = pool.starmap(make_sequence, input_values)
    pool.close()
    pool.join()
